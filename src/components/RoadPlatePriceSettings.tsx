...
import type { RoadPlateRentalPrices, RoadPlatePriceHistory } from '../types';
import { formatPLN } from '../lib/calculations';

interface Props {
  prices: RoadPlateRentalPrices;
  onPricesChange: (prices: RoadPlateRentalPrices) => void;
}

interface FormState {
  base_price_pln: string;
  base_weeks: string;
  price_per_week_1_pln: string;
  threshold_weeks: string;
  price_per_week_2_pln: string;
  note: string;
  loss_price_pln: string;
  service_hour_pln: string;
  sorting_price_pln: string;
  m12_welding_pln: string;
  cutting_head_pln: string;
  lifting_hole_pln: string;
}

function toForm(p: RoadPlateRentalPrices): FormState {
  return {
    base_price_pln: String(p.base_price_pln),
    base_weeks: String(p.base_weeks),
    price_per_week_1_pln: String(p.price_per_week_1_pln),
    threshold_weeks: String(p.threshold_weeks),
    price_per_week_2_pln: String(p.price_per_week_2_pln),
    note: p.note ?? '',
    loss_price_pln: String(p.loss_price_pln),
    service_hour_pln: String(p.service_hour_pln),
    sorting_price_pln: String(p.sorting_price_pln),
    m12_welding_pln: String(p.m12_welding_pln),
    cutting_head_pln: String(p.cutting_head_pln),
    lifting_hole_pln: String(p.lifting_hole_pln),
  };
}

export default function RoadPlatePriceSettings({ prices, onPricesChange }: Props) {
  const [form, setForm] = useState<FormState>(toForm(prices));
  const [saving, setSaving] = useState(false);
  const [history, setHistory] = useState<RoadPlatePriceHistory[]>([]);
  ...
  async function loadHistory() {
    ...
    const { data, error } = await supabase
      .from('road_plate_price_history')
      .select('*')
      .order('changed_at', { ascending: false })
      .limit(20);

    if (!error && data) setHistory(data as RoadPlatePriceHistory[]);
    setLoadingHistory(false);
  }

  async function deleteHistoryEntry(id: string) {
    setDeletingId(id);
    const { error } = await supabase.from('road_plate_price_history').delete().eq('id', id);
  ...
  async function clearAllHistory() {
    const confirm1 = window.confirm(`Usunąć CAŁĄ historię zmian cen płyt?\n\nTa operacja jest nieodwracalna.`);
    if (!confirm1) return;
    ...
    const { error } = await supabase.from('road_plate_price_history').delete().gte('id', '00000000-0000-0000-0000-000000000000');
  ...
  function validate(): string | null {
    ...
    const pw1 = parseFloat(form.price_per_week_1_pln);
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      price_per_week_1_pln: parseFloat(form.price_per_week_1_pln),
      ...
      service_hour_pln: parseFloat(form.service_hour_pln) || 0,
      sorting_price_pln: parseFloat(form.sorting_price_pln) || 0,
      m12_welding_pln: parseFloat(form.m12_welding_pln) || 0,
      cutting_head_pln: parseFloat(form.cutting_head_pln) || 0,
      lifting_hole_pln: parseFloat(form.lifting_hole_pln) || 0,
    };

    const { data, error } = await supabase
      .from('road_plate_rental_prices')
      .update(payload)
      .eq('id', prices.id)
      .select()
      .single();
    ...
    if (error) {
      ...
      onPricesChange(data as RoadPlateRentalPrices);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-1">Cennik wynajmu — płyty drogowe</h2>
        <p className="text-xs text-gray-400 mb-6">
          Ostatnia aktualizacja: {formatDate(prices.updated_at)} · Wszystkie ceny w PLN (toggle EUR w kalkulatorze przeliczy przez kurs)
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          {field('price_per_week_1_pln', `Cena / tydzień (tygodnie ${parseInt(form.base_weeks) + 1 || '?'}–${form.threshold_weeks || '?'}) [PLN/t]`, 'Stawka za każdy tydzień po zakończeniu okresu bazowego, do progu obniżki', 'number', '0.01')}
          {field('threshold_weeks', 'Próg obniżki [tygodni]', 'Po którym tygodniu wchodzi obniżona stawka')}
          ...
          {field('note', 'Notatka (opcjonalna)', 'Opis zmiany cennika, np. "aktualizacja Q2 2026"', 'text')}
        </div>
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
          ...
          <div className="text-sm text-blue-700 space-y-1">
            ...
            <p>• Tygodnie {parseInt(form.base_weeks) + 1 || '?'}–{form.threshold_weeks || '?'}: <strong>+{formatPLN(parseFloat(form.price_per_week_1_pln) || 0)} PLN/t</strong> za tydzień</p>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          <h3 className="text-sm font-semibold text-gray-700 mb-1">Cennik szkód i napraw — płyty drogowe</h3>
          <p className="text-xs text-gray-400 mb-4">
            Stawki drukowane na ofercie w sekcji „Cennik". Zapisywane jako snapshot przy każdej nowej ofercie.
            Wartości w PLN — jeśli oferta jest w EUR, kalkulator przeliczy przez aktualny kurs.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {field('loss_price_pln', 'Zagubienie / strata [PLN/t]', 'Brakujący materiał lub całkowita strata płyt', 'number', '0.01')}
            {field('service_hour_pln', 'Roboczogodzina serwisowa [PLN/h]', 'Np. prostowanie blach', 'number', '0.01')}
            {field('sorting_price_pln', 'Sortowanie i czyszczenie [PLN/t]', 'Koszt sortowania i czyszczenia płyt', 'number', '0.01')}
            {field('m12_welding_pln', 'Spawanie otworów M12 [PLN/szt]', 'Zamknięcie otworu M12 spawem', 'number', '0.01')}
            {field('cutting_head_pln', 'Głowica tnąca [PLN/cięcie]', 'Koszt jednego cięcia głowicą tnącą', 'number', '0.01')}
            {field('lifting_hole_pln', 'Nowy otwór do podnoszenia [PLN/szt]', 'Wykonanie otworu transportowego', 'number', '0.01')}
          </div>
        </div>
              ...
              <tbody>
                {history.map((h) => (
                  <tr key={h.id} className="border-t border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-right text-gray-700">{h.base_price_pln != null ? `${formatPLN(h.base_price_pln)} PLN/t` : '—'}</td>
                    <td className="px-4 py-2.5 text-right text-gray-700">{h.price_per_week_1_pln != null ? `${formatPLN(h.price_per_week_1_pln)} PLN/t` : '—'}</td>
