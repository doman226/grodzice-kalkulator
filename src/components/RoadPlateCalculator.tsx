...
import { calculateRentalCost, calculateRoadPlateMetrics, formatPLN, formatEUR, formatRound, formatNumber, formatRentalPeriod } from '../lib/calculations';
...
export default function RoadPlateCalculator({ profiles, prices, clients, onClientAdded, onOfferSaved }: Props) {
  ...
  const [pricePerWeek1, setPricePerWeek1] = useState<number>(prices.price_per_week_1_pln);
  // Cennik szkód i napraw (override globalnych ustawień dla tej oferty)
  const [lossPrice, setLossPrice] = useState<number>(prices.loss_price_pln);
  const [serviceHourPrice, setServiceHourPrice] = useState<number>(prices.service_hour_pln);
  const [sortingPrice, setSortingPrice] = useState<number>(prices.sorting_price_pln);
  const [m12WeldingPrice, setM12WeldingPrice] = useState<number>(prices.m12_welding_pln);
  const [cuttingHeadPrice, setCuttingHeadPrice] = useState<number>(prices.cutting_head_pln);
  const [liftingHolePrice, setLiftingHolePrice] = useState<number>(prices.lifting_hole_pln);
  ...
  // Reset cen lokalnych gdy globalne się zmienią
  useEffect(() => {
    setPricePerTon(prices.base_price_pln);
    setPricePerWeek1(prices.price_per_week_1_pln);
    setLossPrice(prices.loss_price_pln);
    setServiceHourPrice(prices.service_hour_pln);
    setSortingPrice(prices.sorting_price_pln);
    setM12WeldingPrice(prices.m12_welding_pln);
    setCuttingHeadPrice(prices.cutting_head_pln);
    setLiftingHolePrice(prices.lifting_hole_pln);
  }, [prices.updated_at]);
  ...
  // Transport — identyczna logika jak w grodzicach
  ...
  // Dane do SaveModalu
  const offerItems = useMemo((): RoadPlateOfferItemInput[] =>
  ...
  // Snapshot cennika dla oferty (rp_* z lokalnych state)
  const effectivePricesForOffer = useMemo(() => ({
    base_price_pln: pricePerTon,
    price_per_week_1_pln: pricePerWeek1,
    rp_loss_price_pln: lossPrice,
    rp_service_hour_pln: serviceHourPrice,
    rp_sorting_price_pln: sortingPrice,
    rp_m12_welding_pln: m12WeldingPrice,
    rp_cutting_head_pln: cuttingHeadPrice,
    rp_lifting_hole_pln: liftingHolePrice,
  }), [pricePerTon, pricePerWeek1, lossPrice, serviceHourPrice, sortingPrice, m12WeldingPrice, cuttingHeadPrice, liftingHolePrice]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  >
                    {profiles.map(p => (
                      <option key={p.id} value={p.id}>
                        {p.name} ({p.sheet_width_m}×{p.sheet_length_m} m)
                      </option>
                    ))}
                  </select>
                  {profile && (
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          ...
          {/* Banner sugerowanej ceny — identyczna mechanika jak grodzice */}
          {(() => {
            const suggestedPLN = rentalWeeks <= prices.base_weeks
              ? prices.base_price_pln
              : prices.base_price_pln + (rentalWeeks - prices.base_weeks) * prices.price_per_week_1_pln;
            const suggested = currency === 'EUR' ? suggestedPLN / exchangeRate : suggestedPLN;
            const weekRate = currency === 'EUR'
              ? +(prices.price_per_week_1_pln / exchangeRate).toFixed(2)
              : prices.price_per_week_1_pln;
        ...
        {/* Cennik szkód i napraw — 6 pól specyficznych dla płyt */}
        <div className="mt-4 pt-4 border-t border-gray-100">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              { label: `Roboczogodzina serwisowa [${currency}/h]`, value: serviceHourPrice, set: setServiceHourPrice },
              { label: `Sortowanie i czyszczenie [${currency}/t]`, value: sortingPrice, set: setSortingPrice },
              { label: `Spawanie otworów M12 [${currency}/szt]`, value: m12WeldingPrice, set: setM12WeldingPrice },
              { label: `Głowica tnąca [${currency}/cięcie]`, value: cuttingHeadPrice, set: setCuttingHeadPrice },
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <ResultCard label="Powierzchnia płyt" value={formatNumber(totals.totalAreaM2, 2)} unit="m²" />
              <ResultCard label="Liczba sztuk" value={String(items.reduce((s, i) => s + (i.quantity || 0), 0))} unit="szt." />
            </div>
            {items.length > 1 && (
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <div className="space-y-1">
                  {itemResults.map((r, idx) => r.valid && (
                    <div key={items[idx].uid} className="flex justify-between text-sm text-gray-600">
                      <span>{r.profile!.name} – {items[idx].quantity} szt.</span>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <ResultCard label="Koszt / m²"
                value={formatRound(totals.totalAreaM2 > 0 ? totalCostInclTransport / totals.totalAreaM2 : 0)}
                unit={`${currency}/m²`} />
          ...
          {/* Transport — identyczna sekcja jak w grodzicach */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                {([
                  ...
                  <label key={val} className={`flex-1 flex items-start gap-2.5 p-3 rounded-lg border-2 cursor-pointer transition-colors ${
                    ...
                    <input type="radio" name="transportPaidByRP" value={val} checked={transportPaidBy === val}
      ...
      {showSaveModal && isValid && allItemsValid && transportCalc && (
        <SaveRoadPlateOfferModal
          ...
          totals={{
            ...
            areaM2: totals.totalAreaM2,
            totalLengthM: totals.totalLengthM,
            rentalCostPLN,
            rentalCostEUR,
            costPerM2: totals.totalAreaM2 > 0 ? totalCostInclTransport / totals.totalAreaM2 : 0,
