...
import type { Client, BeamRentalOffer, BeamRentalPrices } from '../../types';
...
export interface BeamOfferItemInput {
  profileId: string;
  profileName: string;
  series: string;
  weightKgPerM: number;
...
interface Props {
  ...
  prices: BeamRentalPrices;       // snapshot cennika szkód (w walucie oferty)
  taskName?: string;
...
export default function BeamSaveOfferModal({
  ...
  async function handleSave() {
    ...
    const { data, error: err } = await supabase.from('beam_rental_offers').insert({
      ...
      // Ceny szkód — w walucie oferty (już przeliczone w kalkulatorze, zapis 1:1)
      ...
      lifting_hole_price_pln: prices.lifting_hole_price_pln,
      // Transport zawsze w PLN w bazie (konwersja z EUR jeśli trzeba)
      delivery_trucks: transport.trucks,
      delivery_cost_per_truck: transport.costPerTruck > 0
        ? (currency === 'EUR' ? transport.costPerTruck * exchangeRate : transport.costPerTruck)
        : null,
      delivery_cost_total: transport.costPerTruck > 0
        ? (currency === 'EUR' ? transport.totalCost * exchangeRate : transport.totalCost)
        : null,
      delivery_paid_by: transport.paidBy,
      delivery_from: transport.from || null,
      delivery_to: transport.to || null,
    ...
    const { data: insertedItems, error: itemsErr } = await supabase.from('beam_rental_offer_items').insert(
      offerItems.map((item, idx) => {
        const valueTotal = item.massT * pricePerTon;
        return {
    ...
    if (itemsErr) {
      // Rollback: soft-delete świeżej oferty (wzorzec SR/SPP — surowy UPDATE deleted_at)
    ...
    <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-0.5">Numer zostanie nadany automatycznie (OH/{new Date().getFullYear()}/XXX)</p>
        </div>
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
            ...
            <div className="space-y-1 mb-3">
              {offerItems.map((item, idx) => (
                <div key={idx} className="flex justify-between text-sm">
                  <span className="text-gray-600">{item.profileName} – {item.quantityPcs} szt. × {item.lengthM} m</span>
            ...
            <div className="pt-2 border-t border-blue-200 mt-2 space-y-1">
              <div className="flex justify-between text-sm">
                ...
                <strong>
                  {currency === 'EUR' ? `${formatEUR(totals.rentalCostEUR)} EUR` : `${formatPLN(totals.rentalCostPLN)} PLN`}
                </strong>
              </div>
              {currency === 'EUR' && (
                <div className="flex justify-between text-sm">
              ...
              {transport.costPerTruck > 0 && transport.paidBy !== 'fca' && (
                <div className="flex justify-between text-sm">
                  ...
                  <strong className={transport.paidBy === 'dap_extra' ? 'text-orange-600' : ''}>
                    {currency === 'EUR' ? `${formatEUR(transport.totalCost)} EUR` : `${formatPLN(transport.totalCost)} PLN`}
                  </strong>
                </div>
              )}
              ...
              <div className="flex justify-between pt-1 border-t border-blue-200 text-sm">
                ...
                <strong className="text-blue-900 text-base">
                  {currency === 'EUR' ? `${formatEUR(totalWithTransport)} EUR` : `${formatPLN(totalWithTransport)} PLN`}
                </strong>
              </div>
            </div>
            {transport.to && (
