...
import type { BeamRentalOffer, BeamProfile, BeamRentalPrices, Client, OfferStatus } from '../../types';
...
import BeamOfferPDF from './BeamOfferPDF';
import type { PdfLang } from '../../lib/pdfStrings';
import BeamEditOfferModal from './BeamEditOfferModal';

interface Props {
  offers: BeamRentalOffer[];
  onOffersChange: (offers: BeamRentalOffer[]) => void;
...
const STATUS_LABELS: Record<OfferStatus, string> = {
  szkic: 'Szkic', wysłana: 'Wysłana', przyjęta: 'Przyjęta', odrzucona: 'Odrzucona',
};
...
function paidByLabel(v?: string): string {
  return v === 'dap_included' ? 'DAP – w cenie'
    : v === 'dap_extra' ? 'DAP – refaktura'
    : v === 'cif' ? 'CIF – odbiór z portu'
    : 'FCA – odbiór własny';
}

export default function BeamOffersTable({ offers, onOffersChange, profiles, prices, clients }: Props) {
  const [search, setSearch]             = useState('');
  const [filterStatus, setFilterStatus] = useState<OfferStatus | 'wszystkie'>('wszystkie');
  const [expanded, setExpanded]         = useState<string | null>(null);
  const [statusSaving, setStatusSaving] = useState<string | null>(null);
  const [pdfLoading, setPdfLoading]     = useState<string | null>(null);
  const [editingOffer, setEditingOffer] = useState<BeamRentalOffer | null>(null);
  const [copyingOffer, setCopyingOffer] = useState<BeamRentalOffer | null>(null);
  ...
  async function handleDownloadPDF(offer: BeamRentalOffer, lang: PdfLang = 'pl') {
    const loadingKey = `${offer.id}-${lang}`;
    setPdfLoading(loadingKey);
    try {
      const blob = await pdf(<BeamOfferPDF offer={offer} lang={lang} />).toBlob();
  ...
  async function changeStatus(offer: BeamRentalOffer, newStatus: OfferStatus) {
  ...
  async function handleDelete(offer: BeamRentalOffer) {
    if (!confirm(`Przenieść ofertę ${offer.offer_number} do kosza?`)) return;
    // Soft-delete — surowy UPDATE deleted_at (belki nie mają RPC soft_delete)
    const { error: err } = await supabase
      .from('beam_rental_offers')
  ...
  const filtered = offers.filter(o => {
    ...
    const matchSearch = !search.trim() ||
      ...
      (o.items ?? []).some(it => it.profile_name.toLowerCase().includes(q));
  ...
  return (
    <div className="space-y-4">
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-green-700 text-white text-sm px-4 py-2 rounded-lg shadow-lg">{toast}</div>
      )}

      <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-wrap">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">Oferty wynajmu dwuteowników (OH)</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              const firstProfile = offer.items && offer.items.length > 0 ? offer.items[0].profile_name : '—';
              return (
              <React.Fragment key={offer.id}>
                <tr className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors`}>
                  <td className="px-4 py-3 font-mono font-semibold text-blue-900">{offer.offer_number}</td>
                  <td className="px-4 py-3">
                    <span className="font-medium text-gray-800">{offer.client?.name ?? <span className="text-gray-400 italic">—</span>}</span>
                  </td>
                  <td className="px-4 py-3 text-gray-500 whitespace-nowrap">{formatDate(offer.created_at)}</td>
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-gray-800">
                    {isEUR && offer.rental_cost_eur != null ? (
                      <div>
                        ...
                        <div className="text-xs font-normal text-gray-400">{formatPLN(offer.rental_cost_pln ?? 0)} PLN</div>
                      </div>
                    ) : (
                      <>{formatPLN(offer.rental_cost_pln ?? 0)} PLN</>
                    )}
                  </td>
                    ...
                    <td colSpan={9} className="px-6 py-4 bg-blue-50 border-t border-blue-100">
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                          ...
                          {isEUR && offer.exchange_rate && (
                            <div><span className="text-gray-500">Kurs EUR:</span> <strong>{offer.exchange_rate.toFixed(4)} PLN</strong></div>
                          )}
                          <div><span className="text-gray-500">Płatność:</span> <strong>{offer.payment_days === 0 ? 'Przedpłata' : `${offer.payment_days ?? 30} dni`}</strong></div>
                          <div><span className="text-gray-500">Ważność:</span> <strong>{offer.valid_days} dni</strong></div>
                          {offer.delivery_cost_per_truck != null && (
                            <div>
                              <span className="text-gray-500">Transport:</span>{' '}
                              <strong className={offer.delivery_paid_by === 'dap_extra' ? 'text-orange-600' : ''}>
                                {isEUR && offer.exchange_rate
                                  ? `${formatEUR((offer.delivery_cost_total ?? 0) / offer.exchange_rate)} EUR`
                                  : `${formatPLN(offer.delivery_cost_total ?? 0)} PLN`}
                                {' '}<span className="font-normal">({paidByLabel(offer.delivery_paid_by)})</span>
                              </strong>
                            </div>
                          )}
                        ...
                        {offer.items && offer.items.length > 0 && (
                          <div className="rounded-lg overflow-hidden border border-blue-200">
                            <table className="w-full text-xs">
                              ...
                              <tbody>
                                {[...offer.items].sort((a, b) => a.sort_order - b.sort_order).map((item, i) => (
                                  <tr key={item.id || i} className={i % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                                    ...
                                    <td className="px-3 py-2 text-right text-gray-600">{item.quantity_pcs}</td>
                        ...
                        {(() => {
                          ...
                          const rentalMain = isEUR
                            ? (offer.rental_cost_eur != null ? formatEUR(offer.rental_cost_eur) : formatEUR((offer.rental_cost_pln ?? 0) / rate))
                            : formatPLN(offer.rental_cost_pln ?? 0);
                          const rentalMainNum = isEUR ? (offer.rental_cost_eur ?? 0) : (offer.rental_cost_pln ?? 0);
                          const transportInCur = offer.delivery_cost_total != null ? (isEUR ? offer.delivery_cost_total / rate : offer.delivery_cost_total) : 0;
                          const totalWithTransport = offer.delivery_paid_by === 'dap_included' ? rentalMainNum + transportInCur : rentalMainNum;
                          const perTon = (offer.total_mass_t ?? 0) > 0 ? totalWithTransport / (offer.total_mass_t ?? 1) : 0;
                          return (
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                              <div className="bg-blue-900 rounded-lg px-3 py-2 text-white">
                                ...
                                {isEUR && (
                                  <p className="text-blue-400 text-xs mt-0.5">≈ {formatPLN(offer.rental_cost_pln ?? 0)} PLN</p>
                                )}
                              </div>
                              <div className="bg-white border border-blue-200 rounded-lg px-3 py-2">
                                <p className="text-gray-400 mb-0.5">Koszt / t</p>
                                <p className="font-semibold text-gray-800">{formatRound(perTon)} {currCode}</p>
                              </div>
                            </div>
                          );
                        })()}
