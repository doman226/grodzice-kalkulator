        ...
        {offer.items && offer.items.length > 0 ? (
          ...
          <View style={s.table}>
            <View style={s.tableHeaderRow} wrap={false}>
              ...
              <Text style={[s.thCell, { flex: 1.1, textAlign: 'right' }]}>{t.thMass}</Text>
              ...
              return [...offer.items].sort((a, b) => a.sort_order - b.sort_order).map((item, idx) => {
                ...
                return (
                  <View key={item.id || idx} wrap={false} style={idx % 2 === 0 ? s.tableBodyRow : s.tableBodyRowAlt}>
                    ...
                    <Text style={[s.tdLabel, { flex: 1.2, color: C.gray700 }]}>{item.steel_grade ?? '—'}</Text>
                    <Text style={[s.tdLabel, { flex: 0.9, textAlign: 'center' }]}>{item.quantity} {t.unitPcs}</Text>
                    <Text style={[s.tdLabel, { flex: 1.0, textAlign: 'right' }]}>{item.length_m != null ? `${item.length_m} m` : '–'}</Text>
                    <Text style={[s.tdLabel, { flex: 1.1, textAlign: 'right', fontFamily: 'Roboto', fontWeight: 700, color: C.gray800 }]}>{formatNumber(item.mass_t, 3)} t</Text>
            ...
            <View wrap={false} style={[s.tableBodyRow, { backgroundColor: C.gray100 }]}>
              ...
              <Text style={[s.tdLabel, { flex: 1.1, textAlign: 'right', fontFamily: 'Roboto', fontWeight: 700, color: C.navy }]}>{formatNumber(offer.mass_t, 3)} t</Text>
