...
import type { Client, BeamProfile, BeamSaleOffer, OfferStatus } from '../../../types';
import { formatEUR, formatPLN, formatNumber } from '../../../lib/calculations';
import { buildPdfFilenameBase } from '../../../lib/pdfFilename';
import BeamSaleEditOfferModal from './BeamSaleEditOfferModal';
import BeamSaleOfferPDF from './BeamSaleOfferPDF';
import type { PdfLang } from '../../../lib/pdfStrings';

interface Props {
  offers: BeamSaleOffer[];
  onOffersChange: (offers: BeamSaleOffer[]) => void;
  clients: Client[];
...
// Marża on-the-fly z pozycji — sell_value_eur / cost_total (cost_total w walucie oferty).
function computeMargin(offer: BeamSaleOffer): { pct: number; amountEUR: number } | null {
  const its = offer.items ?? [];
  const sellEUR = its.reduce((s, i) => s + (i.sell_value_eur ?? 0), 0);
...
export default function BeamSaleOffersTable({
  ...
  const [editOffer, setEditOffer]       = useState<BeamSaleOffer | null>(null);
  const [copyOffer, setCopyOffer]       = useState<BeamSaleOffer | null>(null);
  // pdfLoading format: "offerId-pl" | "offerId-en"
  ...
  async function handleDownloadPDF(offer: BeamSaleOffer, lang: PdfLang = 'pl') {
    const key = `${offer.id}-${lang}`;
    setPdfLoading(key);
    try {
      const blob = await pdf(<BeamSaleOfferPDF offer={offer} lang={lang} />).toBlob();
  ...
  function handleOfferUpdated(updated: BeamSaleOffer) {
    onOffersChange(offers.map(o => o.id === updated.id ? updated : o));
    setEditOffer(null);
    showToast('Oferta zaktualizowana ✓');
  }

  function handleOfferCopied(created: BeamSaleOffer) {
  ...
  async function changeStatus(offer: BeamSaleOffer, newStatus: OfferStatus) {
  ...
  async function handleDelete(offer: BeamSaleOffer) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">Oferty sprzedaży dwuteowników (SH)</h2>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} className="text-center py-12 text-gray-400 text-sm">
                  {search ? 'Brak wyników dla podanego wyszukiwania.' : 'Brak ofert sprzedaży dwuteowników. Użyj kalkulatora, aby stworzyć pierwszą.'}
                </td>
              </tr>
            ) : filtered.map((offer, idx) => {
              const totalMassT = offer.total_mass_t ?? (offer.items ?? []).reduce((s, i) => s + (i.mass_t ?? 0), 0);
              ...
              return (
                <React.Fragment key={offer.id}>
                  <tr className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors`}>
                    {/* Numer */}
                    <td className="px-4 py-3 font-mono font-semibold text-blue-900 whitespace-nowrap">
                      {offer.offer_number || '—'}
                      {offer.is_used && (
                        <span className="ml-1.5 text-xs font-sans font-medium bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full align-middle">
                          używane
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {statusSaving === offer.id ? (
                        ...
                        >
                          {(Object.keys(STATUS_LABELS) as OfferStatus[]).map(st => (
                      ...
                      <td colSpan={8} className="px-6 py-4 bg-blue-50 border-t border-blue-100">
                        <div className="space-y-3">
                          <p className="text-xs font-semibold text-blue-800 uppercase tracking-wide">
                            ...
                            {offer.is_used && <span className="ml-2 normal-case font-medium text-amber-700">⚠️ dwuteowniki używane</span>}
                          </p>
                              ...
                              <table className="w-full text-xs">
                                <thead>
                                  <tr className="bg-blue-800 text-white">
                                    ...
                                    <th className="text-right px-3 py-2 font-semibold">Dł. [m]</th>
                                  ...
                                  {offer.items
                                    ...
                                    <tr key={item.id} className={i % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                                      <td className="px-3 py-2 font-semibold text-gray-800">
                                        {item.profile_name}
                                        <span className="ml-1 font-normal text-gray-400">({item.series})</span>
                                      </td>
                                      <td className="px-3 py-2 text-gray-600">{item.steel_grade}</td>
                                      <td className="px-3 py-2 text-right text-gray-600 tabular-nums">{item.quantity_pcs}</td>
                                      <td className="px-3 py-2 text-right text-gray-600 tabular-nums">{item.length_m}</td>
                                      <td className="px-3 py-2 text-right text-gray-600 tabular-nums">{formatNumber(item.weight_kg_per_m, 2)}</td>
                                      <td className="px-3 py-2 text-right text-gray-600 tabular-nums">{formatNumber(item.mass_t, 3)}</td>
                                      <td className="px-3 py-2 text-right font-semibold text-gray-800 tabular-nums">{formatNumber(item.sell_per_ton, 2)}</td>
                                      <td className="px-3 py-2 text-right font-bold text-gray-900 tabular-nums">
                                        {currency === 'PLN'
                                          ? `${formatPLN(item.sell_value_pln)} PLN`
                                          : `${formatEUR(item.sell_value_eur)} EUR`}
                                      </td>
                                    </tr>
                                  ))}
                                </tbody>
                                <tfoot>
                                  <tr className="bg-blue-50 border-t border-blue-200">
                                    ...
                                    <td className="px-3 py-2 text-right font-bold text-blue-900 tabular-nums">
                                      {currency === 'PLN'
                                        ? `${formatPLN(offer.items.reduce((s, i) => s + (i.sell_value_pln ?? 0), 0))} PLN`
                                        : `${formatEUR(offer.items.reduce((s, i) => s + (i.sell_value_eur ?? 0), 0))} EUR`}
                                    </td>
                                  </tr>
                                </tfoot>
                              </table>
                          ...
                          <div className="flex justify-between pt-2 border-t border-blue-200">
                            ...
                            <div className="flex gap-2">
                              ...
                              {/* Kopiuj — otwiera modal edycji w trybie copy (nowy numer SH/YYYY/NNN) */}
                              <button
