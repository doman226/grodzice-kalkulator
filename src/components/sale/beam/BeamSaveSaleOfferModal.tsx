...
import type { Client, BeamSaleOffer, BeamSaleOfferItem, OfferStatus } from '../../../types';
...
export default function BeamSaveSaleOfferModal({
  ...
  // FCA pre-fill z kalkulatora: pole „Odbiór z (magazyn)" trafia jako domyślna lokalizacja
  ...
  async function handleSave() {
    ...
    const savedOffer = data as BeamSaleOffer;
    ...
    const { data: insertedItems, error: itemsErr } = await supabase
      ...
      .insert(
        items.map((item, idx) => ({
          ...
          series:          item.series,
          weight_kg_per_m: item.weightKgPerM,
          steel_grade:     item.steelGrade,
          quantity_pcs:    item.quantityPcs,
          length_m:        item.lengthM,
          total_length_m:  item.totalLengthM,
          mass_t:          item.massT,
          cost_per_ton:    item.costPerTon,
          sell_per_ton:    item.sellPerTon,
          cost_total:      item.costTotal,
          sell_total:      item.sellTotal,
          sell_value_eur:  item.sellValueEUR,
          sell_value_pln:  item.sellValuePLN,
    ...
    if (itemsErr) {
      // Rollback: soft-delete świeżej oferty (konwencja beam — nie hard delete)
    ...
    setSaving(false);
    savedOffer.items = (insertedItems ?? []) as BeamSaleOfferItem[];
    onSaved(savedOffer);
  }
  ...
  const totalForClientCurrency = (currency === 'EUR'
    ? totals.totalSellEUR
    : totals.totalSellPLN) + deliveryCostCurrency;
    ...
    <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-gray-100">
          <h3 className="text-lg font-semibold text-gray-800">Zapisz ofertę sprzedaży dwuteowników</h3>
          <p className="text-xs text-gray-400 mt-0.5">SH/{year}/XXX – numer nadany automatycznie</p>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
            <p className="text-xs text-blue-700 font-medium uppercase tracking-wide mb-2">
              Dwuteowniki ({items.length} poz.)
            </p>
            <div className="space-y-1 mb-3">
              {items.map((item, idx) => (
                <div key={idx} className="flex justify-between text-sm">
                  <span className="text-gray-600">
                    {item.profileName} – {item.quantityPcs} szt. × {item.lengthM} m
                    <span className="text-gray-400 ml-1">({item.steelGrade})</span>
                  </span>
                  <span className="font-medium text-gray-800">
                    {formatNumber(item.massT, 3)} t · {formatEUR(item.sellValueEUR)} EUR
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="pt-2 border-t border-blue-200 mt-2 space-y-1">
              ...
              {delivery && (
                <>
                  {delivery.costPerTruck > 0 && delivery.paidBy !== 'fca' && delivery.paidBy !== 'cif' && (
                    <div className="flex justify-between text-sm pt-1 border-t border-blue-200">
                  ...
                  {delivery.paidBy === 'cif' && (
                    <div className="flex justify-between text-sm pt-1 border-t border-blue-200">
                      <span className="text-gray-500">Dostawa:</span>
                      <strong className="text-green-700">CIF – odbiór z portu</strong>
                    </div>
                  )}
                  ...
                  {(delivery.from || delivery.to) && delivery.paidBy !== 'fca' && delivery.paidBy !== 'cif' && (
                    <p className="text-xs text-gray-500 mt-1">
                      🚛 {delivery.from}{delivery.to ? ` → ${delivery.to}` : ''}
                    </p>
                  )}
                  {(delivery.paidBy === 'fca' || delivery.paidBy === 'cif') && delivery.from && (
                    <p className="text-xs text-gray-500 mt-1">
                ...
                <div className="flex gap-4 mb-3">
                  {([
                    ...
                    <label key={opt.val} className="flex items-center gap-1.5 cursor-pointer text-sm">
                      <input type="radio" name="beamSaleDeliveryTimeline" value={opt.val}
              ...
              <div>
                ...
                <div className="flex gap-4 mb-3 flex-wrap">
                  {([
                    ...
                    <label key={opt.val} className="flex items-center gap-1.5 cursor-pointer text-sm">
                      <input type="radio" name="beamSaleDeliveryTerms" value={opt.val}
                ...
                {deliveryTerms === 'FCA' && (
                  <div>
                    ...
                    <input type="text" value={fcaLocation}
                      ...
                      placeholder="np. Cieśle 42, 56400, PL"
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    <p className="text-xs text-gray-400 mt-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <label key={opt.val}
                      ...
                      <input type="radio" name="beamSalePaymentDays" value={opt.val}
        ...
        <div className="p-6 border-t border-gray-100 flex justify-end gap-3">
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? 'Zapisywanie...' : 'Zapisz ofertę SH'}
          </button>
        </div>
      </div>
    </div>
